...
dash_push:paint(5, "Welcome to Naxuy")
...
local menu = {
    welcome = {
        asdsadsa = aa_group:label(
            '         \a7DBABCC4• · . · \aFFFFFFFF✧˚ \a7DBABCC4Alpha  \aFFFFFFFF˚ ✧ \a7DBABCC4· . · •'),
        name = aa_group:label('                     Legacy Alpha \a7DBABCC4developer 333qq and batya'),
        welcome = aa_group:label("              welcome back, \a7DBABCC4" .. cvar.name:get_string()),
        made = aa_group:label('          made by \a7DBABCC4333qq \aFFFFFFFFfucking coder'),
        curbuuld = aa_group:label('               current build: \a7DBABCC4recode '),
        night2024 = fakelag_group:label('              \a7DBABCC4Legacy 20\aFFFFFFFF24 Gey'),
        Lastup = fakelag_group:label('       last update: \a7DBABCC430.03.2025'),
        curscript = fakelag_group:label('    current script: \a7DBABCC4Legacy \aFFFFFFFF~ ALPHA'),
        asdasdhh = aa_group:label('-------------------------------------------------'),
        cfggaa = aa_group:label('          \a7DBABCC4dont forget to \a7DBABCC4save the \a7DBABCC4config'),

        cfgaaaaa = aa_group:label('Gamesense shit paste'),
        cfgaaaaaa = aa_group:label('update 24 04 27 mb'),
    ...
    navigation = {
        ...
        tab = fakelag_group:combobox('Navigation',
            { ' \a7D7D7DFFinformation', ' \aFFC0CBFFglobal', ' \a7D7D7DFFantiaim', ' \aFFC0CBFFvisuals',
                ' \a7D7D7DFFmisc' })
    },
    global = {
        lines21 = aa_group:label("        \a7DBABCC4Legacy ~ \aFFFFFFFFconfigurations"),
    ...
    antiaim = {
        nightasa = aa_group:label("              \a7DBABCC4Legacy Alpha ~ \aFFFFFFFFanti-aims"),
    ...
    visuals = {
        nightvisuals = aa_group:label("                \a7DBABCC4Legacy ~ \aFFFFFFFFvisuals"),
        ...
        target_active = aa_group:checkbox(
            "\v\r~ \a7DBABCC4resolved \aFFFFFFFFlist \aFFFFFFFF(turn on \aFFFFFFFFLegacy\a7DBABCC4solver)"),
    ...
    misc = {
        nightmisc = aa_group:label("              \a7DBABCC4Legacy ~ \aFFFFFFFFmiscellaneous"),
        resolver = aa_group:checkbox("\v\r~ \aFFFFFFFFLegacy\a7DBABCC4solver"),
...
shot_logger.on_aim_hit = function(e)
    ...
    if menu.visuals.aimbot_logs:get("~console") then
        shot_logger.add({ info.prefix[1], info.prefix[2], info.prefix[3], '[Legacy]' },
            { 134, 134, 134, ' » ' },
...
function crosshair()
    ...
    if menu.visuals.crosshair_style:get() == "~static" then
        renderer.text(x * 0.5 + scoped_space, y * 0.5 + 4.5 + y_offset, r, g, b, 255, 'cbd', nil, 'Legacy')
    elseif menu.visuals.crosshair_style:get() == "~gradient" then
        renderer.text(x * 0.5 + scoped_space, y * 0.5 + 4.5 + y_offset, r, g, b, 255, 'cbd', nil,
            gradient_text2('Legacy ALPHA', globals.curtime() * 0.8, r, g, b, 255, 255, 255, 255, 255))
    end
...
function watermark()
    ...
    local text2 = strange('Legacy ALPHA', globals.curtime() * 6)
    ...
    if menu.visuals.watermark:get() == '~gradient' then
        local text = gradient_text('L E G A C Y', globals.curtime(), r, g, b, 255, 155,
            155, 155, 200)
        local tx, ty = renderer.measure_text('cd',
            string.format('%s\a%s %s', text, to_hex(r, g, b, 200), '[ALPHA]'))
        renderer.text(x - tx / 2 - 1824, y / 2, r, g, b, 200, 'cd', nil,
            string.format('%s\a%s %s', text, to_hex(r, g, b, 200), '[ALPHA]'))
    end
    if menu.visuals.watermark:get() == '~modern' then
        local png = images.load_png(readfile('night.png'))
        local text = gradient_text('Private', globals.realtime(), r, g, b, 255, 155,
            155, 155, 200):upper()
        png:draw(15, y / 2 + 15, 25, 25, r, g, b)
        renderer.text(45, y / 2 + 20, 255, 255, 255, 200, '-', nil,
            string.format('Legacy ALPHA [%s\aFFFFFFFF]', text):upper())
...
local killsays = { 'youtube/@Roma_CSGO_Cheats921',
    'Walk bots vs Творцы',
    "Оутлав имба имба,кряк надо качать",
    "Нижняя пышма дом 71",
    "встань на правильный путь,встань с бутылки",
    'xd?' }
